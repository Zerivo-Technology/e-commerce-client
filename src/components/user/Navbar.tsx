...
import { faBars, faBarsStaggered, faCartShopping, faSearch } from "@fortawesome/free-solid-svg-icons";
        ...
        <div className="h-14 w-full flex items-center justify-center font-monserat border-b">
            <div className="flex container h-full items-center justify-between">
                <div className="flex items-center gap-14">
                    ...
                    <div className={`hidden md:flex items-center gap-4 text-sm`}>
                        {navLinks.map((link, index) => (
                            link.label === "Shop" ? (
                                <DropdownMenu key={index} title={link.label} items={dropdownItems} />
                            ) : (
                ...
                <div className="flex items-center gap-4 text-primary-color">
                    <Link to="/sign-in" className="flex items-center gap-2 text-xs font-semibold cursor-pointer hover:text-hover-color">
                        <FontAwesomeIcon icon={faUser} />
                        <p>Login/Register</p>
                    </Link>
                    <FontAwesomeIcon icon={faSearch} className="text-xs cursor-pointer hover:text-hover-color" />
                    <div className="flex items-center gap-1 text-xs cursor-pointer hover:text-hover-color">
